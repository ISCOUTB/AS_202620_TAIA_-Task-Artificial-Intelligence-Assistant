...
|---|---|---|---|
| **AI → Academic** | Cliente / Proveedor + Capa Anticorrupción | AI define `AcademicGateway` y `AcademicGatewayAdapter` para traducir las operaciones académicas al modelo utilizado por AI. La implementación vigente consume `AcademicTaskManagement` mediante `AcademicGatewayAdapter`, que traduce `AcademicTaskData` al modelo de AI. | AI consume operaciones académicas mediante el contrato del proveedor y no depende del repositorio ni de la entidad `Task`. |
| **Reminders → Academic** | Cliente / Proveedor | Reminders necesita comprobar que una tarea pertenece al estudiante y asociar el recordatorio a esa tarea. La implementación vigente consume `AcademicTaskLookup` mediante `AcademicTaskLookupAdapter`; el repositorio permanece encapsulado en Academic. | Reminders solicita una consulta académica mediante el contrato `AcademicTaskLookup` y no importa el repositorio interno. |
...
La **capa anticorrupción (ACL)** se utiliza cuando una dependencia externa o un contexto proveedor puede introducir un modelo que no debe filtrarse al contexto consumidor. En TAIA esto se refleja especialmente en el `AcademicGatewayAdapter` de AI y en los adaptadores de Telegram. Las relaciones con Academic y Usuario se implementan mediante contratos de aplicación (`AcademicTaskLookup`, `AcademicTaskManagement` e `IdentityService`), mientras los adaptadores internos permanecen encapsulados en sus contextos.
